...
// File: listl.cpp   define schedule algorithm List_L's functions
//
//---------------------------------------------------------------------------------------------------------------------//
...
ListL::ListL(int mul,int add_sub,int logic)
{
    this->multi_num =mul;
	this->add_sub_num =add_sub;
	this->logic_num =logic;

	this->multi_max =mul;//max_avaliable
	this->add_sub_max =add_sub;//max_avaliable
	this->logic_max =logic;//max_avaliable
...
vertex::vertex(const string &com,int order, int input,vector<string> statement,int row_num)
{
...
edge::edge(const string &value, int order)
{
    this->value =value;
	this->order =order;
...
void  vertex::WriteVex_height(int height)
{
	this->height=height;
}

// write into the private elements
void  vertex::WriteVex_hardware(string hardware)
{
	this->hardware=hardware;
}

// write into the private elements
void  vertex::WriteVex_cylces(int cylces)
{
	this->cylces=cylces;
}
void  vertex::WriteVex_Starttimer(int Starttimer)
{

		this->Starttimer=Starttimer;
}
void  vertex::WriteVex_Endtimer(int Endtimer)
{
...
bool ListL::ListlAlgorithm(char* filename,PublicFunction pubfun)
{
...
//	Big loop
				...
				if(this->flag_empty_graph!=1){  //it isnot an empty graph
					...
					while(Finishtime+1 != timer)
					{
							if(Finishtime>=end_state)
							{
								...
								vertex* finishindex=this->GetVex_head();
	...
	cout<<"listl HLSM Finished£¡"<<endl;
						
	fp.close();
	return true;
}

void ListL::Clearvertex()
{
	edge *tempedge =GetEdge_head();
	edge *oldedge=tempedge;

	vertex* temp= GetVex_head();
	vertex* old=temp;
	...
	this->finish=0;//Finish signal for the processor
	this->multi_num =this->Get_mul_max();
	this->add_sub_num =this->Get_add_sub_max();
	this->logic_num =this->Get_add_sub_max();
...
vertex* ListL::CreateGragh(vector<vector<string> > ScheObj,PublicFunction pubfun)
{
	vertex *node=NULL;
	vertex *curr_vex=NULL; //node is used for creating new vertex, curr_vex is used for indicate which vertex
	//vertex * OPER_temp=NULL;
	//vertex * REG_temp=NULL;
	edge   *side=NULL;
	edge *curr_edge=NULL; //side is used for creating new edge, curr_edge is used for indicate which edge
	int index_vertex =1;
	int index_edge =1;
	vector<string> empty;
	empty.push_back("");

	node =new vertex("nop_start",0,0,empty,-1); //nop_start is node_order =0, InputNum =0
	this->Vex_head =node; //connect the head of graph
	curr_vex =node;

	node =new vertex("nop_end",-2,pubfun.Output.size(),empty,-1);  //nop_end is node_order =MAX_order,InputNum= output node Num
	...
	curr_vex =node;		
	 ...
	 for(unsigned int j=0;j<ScheObj.size();j++)//Hight(Y) ->Equation
		{	
			 bool flag_circle =0;
			 edge * log_sear_outedge=NULL;
			 vertex * OPER_temp=NULL;
	         vertex * REG_temp=NULL; // initial
			...
			//create a operation vertex if that operation is not "="
				 ...
				 node =new vertex(ScheObj.at(j).at(3),index_vertex,(ScheObj.at(j).size()-1)/2,ScheObj.at(j),j);  
				 index_vertex++;
           ...
           ///////////////// check if the outcome is OUTPUTS ///////////////////
		    if (Sear_list(pubfun.Output,ScheObj.at(j).at(0))){
				if(ScheObj.at(j).size()==3){ //z=zin, it will not have operation vertex
					...
					node =new vertex("REG", index_vertex,1,ScheObj.at(j),j);
					REG_temp =node;
					index_vertex++;
					...
					side =new edge(ScheObj.at(j).at(0),index_edge);
					index_edge++;
				...
				// create edge go out from operation vertex
					...
					side =new edge(ScheObj.at(j).at(0),index_edge); // delaytime of edge is 0
					index_edge++;
			...
			while (i<ScheObj.at(j).size()){
			   if (Sear_list(pubfun.Input,ScheObj.at(j).at(i))){
					...
					side =new edge(ScheObj.at(j).at(i),index_edge);
					side->node_in =node->GetVex_order();
					...
					side =new edge(ScheObj.at(j).at(i),index_edge); // delaytime of edge is 0
					index_edge++;
					...
					vector<edge*> log_muldummy;
					edge *depend =this->GetEdge_head();
                    edge *lastline =NULL;
					...
					if(lastline!=NULL){
						if(lastline->node_in==Hanging){
							...
							for(unsigned int kk=0;kk<log_muldummy.size();kk++){
								node =new vertex("DUMMY",index_vertex,1,empty,-1);
								index_vertex++;
								node->node_prev =curr_vex; // connect to vertex list
								curr_vex->node_next =node;
								curr_vex =node;
								// create a edge go from dummy node, go into opea/reg node
								side =new edge("DUMMY",index_edge); // delaytime of edge is 0
								index_edge++;
								if (this->edge_head ==NULL){
									this->edge_head =side;
				 					side->edge_prev =this->edge_head;
					...
					edge *depend_out2 =this->GetEdge_head();
					edge *lastline_normal =NULL;
			...
			if(log_del.size()!=0){ // delete the connected edge
				for(unsigned int k=0;k<log_del.size();k++){
					if(log_del.at(k)==this->GetEdge_head()){
						...
						edge* del_next =log_del[k]->edge_next;
						edge* del_prev =log_del[k]->edge_prev;
			...
			vector<edge*> log_muldummy_normal;
			edge *comm =this->GetEdge_head();
			edge *lastline_comm =NULL;
			while(comm!=NULL){
			...
			edge *overlap =this->GetEdge_head();
			vector<edge *> comm_overlap;
			while(overlap!=NULL){
    ...
    vertex *traver_vertex =this->GetVex_head();
	edge *traver_edge =this->GetEdge_head();
	edge *link_edge_in =NULL;
	edge *link_edge_out =NULL;
	...
	vertex* trave_empty =this->GetVex_head();
	int count_empty =0;
...
bool ListL::Sear_list(vector<string> netlist, string element)  
{
     unsigned  int i=0;
	if (netlist.size()==0){
		return 0;
...
bool ListL::Delete_edge()
{
    ...
    edge* index =this->GetEdge_head();
	edge* deletedge =NULL;
	edge* temp_prev=NULL,*temp_next=NULL;
	edge* temp_location=NULL;
	while(index!=NULL){		
...
vertex* ListL::Vertex_Finder(int ID)
 {
    vertex* temp_vertex=this->GetVex_head();
...
vertex*	ListL::Find_AdjVertex_Base_edgeup(edge* index_edge)//Up tree use
{
	//From down to up
	...
	if(index_edge == NULL)//We explored all the adj. vertex
		...
		{
			vertex_id = index_edge->node_out;//Up tree use this
...
vertex*	ListL::Find_AdjVertex_Base_edgedown(edge* index_edge)//down tree uses old
{
	//From up to down
...
bool ListL::detect_hardware()
{
	//cout<< endl;
	vertex* index=this->GetVex_head();
	while(index!=NULL){
...
int ListL::detect_finish()
{
	vertex* index_vertex=this->GetVex_head();//up
	while(index_vertex!= NULL)
	{
		if(index_vertex->GetVex_cylces()!=0)//Check all the components that with a height
		{
...
//ASAP(Up to Down)
void ListL::CreateDownTree()//ASAP
{	
	vertex* index_vertex=this->GetVex_head();//up
	vertex* temp=NULL;
	edge* edgeout_counter;//=index_vertex->edge_in;//get edges linkto the pre
	while(index_vertex!= NULL)
	{
		edgeout_counter=index_vertex->edge_out;
		while(edgeout_counter!=NULL)
			{				

				temp=Find_AdjVertex_Base_edgedown(edgeout_counter);//find the actural vertex
				temp->up=index_vertex;
				index_vertex->DownTree.push_back(temp);
				edgeout_counter=edgeout_counter->edge_OutNext;//Find the other branch that goes out
	...
	downroot=this->GetVex_head();//Down tree's root is the head

}

void ListL::Sort_DownTree()
{
	vertex* index_vertex=this->downroot;//Header
	Sort_DownTree_Caculator(index_vertex);


}

void ListL::Sort_DownTree_Caculator(vertex* node)
{
	//int temp=0;
	if( node->DownTree.size() == 0) 
		return;
	else{
		unsigned int i=0;

		this->visit_DownTree(node);
		while(node->DownTree.size()>i)
		{				
			Sort_DownTree_Caculator(node->DownTree.at(i));
			i++;
		}
		//cout<<endl;
	}
...
void ListL::visit_DownTree(vertex* index_vertex)
{
		vertex* UpVertex=NULL;
		int temp=-1;
		//edge* index_edge=index_vertex->edge_in;//get edges
		edge* temp_edge_in=NULL;

		temp_edge_in=index_vertex->edge_in;
		UpVertex=Find_AdjVertex_Base_edgeup(temp_edge_in);
		
		while(temp_edge_in!= NULL)//Go through all the output edges per vertex
		{
				if( UpVertex != NULL )
				{
					//Keep the next one.
					if(index_vertex->GetVex_cylces()!=0)
					{
						if(UpVertex->GetVex_height()>=index_vertex->GetVex_height())//Compare with all out_edge's height, pick the heightest one
						{
							temp=UpVertex->GetVex_height();//Get the Next height
							temp++;//Update the height
			...
			temp_edge_in=temp_edge_in->edge_InNext;
			UpVertex=Find_AdjVertex_Base_edgeup(temp_edge_in);
		}
...
void ListL::CreateUpTree()//ALAP
{
	vertex* index_vertex;
	//vertex* indicator=NULL;
	vertex* temp=NULL;
	vector<string> empty;
	edge* edgein_counter;//=index_vertex->edge_in;//get edges linkto the pre
	edge* indexedge=NULL;
	int inputsnumber=0;
	int edgeID=-100;
...
/*	cout<<"before:"<<endl;
	...
	vertex* newuproot=new vertex("uproot",-5,inputsnumber,empty,-1);//up
	vertex* temphead =this->GetVex_head();
	while(temphead!= NULL){
		...
		if(temphead->GetVex_comp()!="nop_end" && temphead->GetVex_comp()!="nop_start"){
		   if(temphead->edge_out==NULL){		
			  // for the new edge
			  edge* side =new edge(temphead->GetVex_comp(),edgeID); 
	          edgeID++;
			  ...
			  if(this->GetVex_tail()->edge_in==NULL){  // no valid nop_end
				  ...
				  edge* find_edgeend =this->GetVex_tail()->edge_in;
				  edge* find_prev=this->GetVex_tail()->edge_in;
    ...
    if(this->GetVex_tail()->edge_in==NULL){  // has update
		vertex* old_end =this->GetVex_head()->node_next;   // old nop_end
		vertex* old_end_next =old_end->node_next;
...
/*	cout<<"after:"<<endl;
	...
	vertex* newupend=new vertex("upend",-6,inputsnumber,empty,-1);//up
	newupend->WriteVex_cylces(0);
	edge* tempdege =this->GetEdge_head();
    vertex* new_nop_start=NULL;  // it is what you want
	while(tempdege!= NULL){  // travse the who graph, finding the node without edge_in
			...
			if( tempdege->node_out==Hanging ){	
				if(this->GetVex_head()->edge_out==NULL ){  // invalid old nop_start
					...
					edge* find_edgestart =this->GetVex_head()->edge_out;
					edge* find_prevs=this->GetVex_head()->edge_out;
...
void ListL::Sort_UpTree()
{
	//vertexR* index_vertex=this->uproot;
	vertex* index_vertex=this->uproot;
	Sort_UpTree_Caculator(index_vertex);
}

void ListL::Sort_UpTree_Caculator(vertex* node)
{	
	//int temp=0;
...
void ListL::visit_UpTree(vertex* index_vertex)
{
	vertex* NextVertex=NULL;
	int temp=-1;
	int tempcycle=0;
//	edge* index_edge=index_vertex->edge_out;//get edges
	edge* temp_edge_out=NULL;
				...
				if( NextVertex != NULL )
				{
					...
					if(index_vertex->GetVex_cylces()!=0 || index_vertex->GetVex_comp()=="DUMMY")
					{
						if(NextVertex->GetVex_height()>=index_vertex->GetVex_height())//Compare with all out_edge's height, pick the heightest one
						{
							...
							{
								temp=NextVertex->GetVex_height();//Get the Next height
								temp++;//Update the height
								index_vertex->WriteVex_height(temp);//Update
								tempcycle=index_vertex->GetVex_cylces();
...
void ListL::Schedule_Add(int timer)
{
	int maxindex=-1;
...
//	bool check=false;
	...
	vertex* index_vertex=this->GetVex_head();
	vector<vertex*> HeightSort;//Sort out the Descencing order.
	...
	while(index_vertex!=NULL)//Go through all the vertex
	{
		...
		for(count=0;count<index_vertex->UpTree.size();count++)
			...
			vertex* index_left=this->GetVex_head();
			edge* tempedge=NULL;
			index_left=this->GetVex_head();
			tempedge=index_left->edge_in;//
	
			while( index_left!= NULL)
			{	
				while(tempedge!= NULL)//Check all the output
				{
					if(index_left->edge_out!= NULL && 
					 tempedge->GetEdge_value() == index_vertex->statement[0] 
					 && index_left->row_num < index_vertex->row_num && index_left->Done!="done")//all the left
					{
						sizecouter--;				
					}
			
					tempedge=tempedge->edge_InNext;// find next variables on the right
				}

				index_left =index_left->node_next;
				if(index_left!= NULL)
				{
					tempedge=index_left->edge_in;
				}	
	...
	//cout<<endl<<"The Add source avaialble is "<< source <<" timer is: "<<timer<<endl;
	
}

void ListL::Schedule_Logic(int timer)
{	
	unsigned int sizecouter=0;
	int maxindex=-1;
	unsigned int count=0;
...
void ListL::Schedule_Multi(int timer)
{
	unsigned int sizecouter=0;
	int maxindex=-1;
	unsigned int count=0;
	...
	//	cout<<endl<<"The Multi source avaialble is "<<source<<" timer is: "<<timer<<endl;
}

void ListL::Schedule_Reg(int timer)
{
...
void ListL::Reg_Release(int timer)
...
void ListL::Add_Release(int timer)
{
	//int maxindex=-1;
//	int count=0;
//	bool check=true;
...
void ListL::Logic_Release(int timer)
{
//	int maxindex=-1;
//	int count=0;
//	bool check=true;
...
void ListL::Multi_Release(int timer)
{
//	int maxindex=-1;
//	int count=0;
//	bool check=true;
...
int ListL::Find_Maxheight(vector<vertex*> HeightSort)
{
	int tempcount=0;
	int tempmaxheight=0;
...
void ListL::printout_VerList_Backward()
{

	cout<< endl;
	vertex* index=this->GetVex_tail();
	cout<<" vertex_order_backward"<<endl;
	while(index!=NULL){
		cout<<" vertex_order:"<<index->GetVex_order()<<" component: "<<index->GetVex_comp()<<" "<< endl;
		index =index->node_prev;
	}

}

//printout_EdgeList() this is a test function,by printing out all the edges
void ListL::printout_EdgeList()
{
	cout<<endl;
	edge* index= this->GetEdge_head();
	while(index!=NULL){
...
void ListL::printout_max_hardware()
{
	cout<<" max multi; "<<this->Get_mul_max()<<" max adder; "<<this->Get_add_sub_max()<<" max logic; "<<this->Get_logic_max()<<endl;
	
}	
//printout_OutLink() this is a test function,by printing out all the edges that go out the vertex
void ListL::printout_OutLink()
{
	cout<<endl;
	cout<<"the edges go out from vertex:"<<endl;
    vertex* index_vertex=this->GetVex_head();
	edge* index_edge=NULL;
	while(index_vertex!=NULL){
...
void ListL::Rel_Memory()
{
	...
	vertex* indexV=this->GetVex_head();
	vertex* dele_vertex =NULL;
	while(indexV!=NULL){
		dele_vertex =indexV;
